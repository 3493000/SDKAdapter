...
namespace Hunter
{
    ...
    public partial class FacebookSocialAdapter : TSingleton<FacebookSocialAdapter>
    {
        ...
        {
            get { return isLoggedIn && m_RankDataTimeRecorder.needRefresh; }
        }

        public bool isFriendsDataNeedRefresh
        {
            get { return isLoggedIn && m_FriendsDataTimeRecorder.needRefresh; }
        }
...
//        }
        ...
        private void ProfilePhotoCallback(IGraphResult result)
        {
            if (!string.IsNullOrEmpty(result.Error))
            {
                Log.e("Retrieve Profile Photo Error" + result.Error);
                EventSystem.S.Send(SDKEventID.OnFBRetrievePhoto, false);
                return;
            }

            if (result.Texture != null)
            {
                EventSystem.S.Send(SDKEventID.OnFBRetrievePhoto, true, result.Texture);
            }
            else
            {
                Log.e("Retrieve Profile Photo Null");
                EventSystem.S.Send(SDKEventID.OnFBRetrievePhoto, false);
            }
        }
