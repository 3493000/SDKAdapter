...
namespace Hunter
{
    ...
    public class DataAnalysisMgr : TMonoSingleton<DataAnalysisMgr>, IDataAnalysisAdapter, IPanelEventLogger
    {
        ...
        public void OnApplicationQuit()
        {
            ...
            CustomEventWithAppsflyer(DataAnalysisDefine.PLAY_TIME, null, new Dictionary<string, string>()
            {
                { "time", (time % 30).ToString() }
            });

            CustomEventWithAppsflyer(DataAnalysisDefine.W_ENGAGEMENT, null, new Dictionary<string, string>()
            {
                { "time", time.ToString() }
            });
        ...
        public void CustomEvent(string eventID, string label = null, Dictionary<string, string> dic = null)
        {
            ...
            for (int i = 0; i < m_Adapters.Count; ++i)
            {
                if (m_Adapters[i].Platform != "appsflyer")
                {
                    m_Adapters[i].CustomEvent(eventID, label, dic);
                }
            }
        }

        public void CustomEventWithAppsflyer(string eventID, string label = null, Dictionary<string, string> dic = null)
        {
            for (int i = 0; i < m_Adapters.Count; ++i)
            {
                m_Adapters[i].CustomEvent(eventID, label, dic);
        ...
        public void CustomEventLifeCircleSingle(string eventID, string label = null)
        {
            ...
            CustomEventWithAppsflyer(eventID, label);
        }
        ...
        protected void InitDataAnalysisEnv()
        {
            ...
            {
                CustomEventWithAppsflyer("w_app_start", "coldLa");
                CustomEventDailySingle("Retain", null, false);
                CustomEventDailySingle("RetainDaily", "Retain_" + m_InstallDaysStringMax);
                CustomEventLifeCircleSingle("w_first_install");
                CustomEventExtension();
            }, 15);

            EventSystem.S.Register(EngineEventID.OnApplicationFocusChange, OnFocusChanges);
        }

        private void OnFocusChanges(int key, params object[] param)
        {
            if ((bool)param[0])
            {
                CustomEventWithAppsflyer("w_app_start", "hotLa");
            }
        }
