    ...
    */
    ...
    public class PurchaseMgr : TMonoSingleton<PurchaseMgr>
    {
        ...
        public void DoPurchase(TDPurchase data)
        {
            ...
            DataAnalysisMgr.S.CustomEventWithAppsflyer(DataAnalysisDefine.PURCHASE_REQUEST, null, new Dictionary<string, string>()
            {
                {"item_id", data.id},{"value", ((float)data.price / 100).ToString()}
            });
        }
        ...
        private class PurchaseSuccessTask : IThreadTask
        {
            ...
            public bool Execute()
            {
                ...
                DataAnalysisMgr.S.CustomEventWithAppsflyer(DataAnalysisDefine.PURCHASE_SUCCESS, null,new Dictionary<string, string>()
                {
                    {"item_id", data.id},{"value", ((float)data.price / 100).ToString()}
                });
        ...
        public void OnProductPurchaseCancled(Product product, string res)
        {
            ...
            DataAnalysisMgr.S.CustomEventWithAppsflyer(DataAnalysisDefine.PURCHASE_FAILED, res, new Dictionary<string, string>()
            {
                {"item_id", data.id},{"value", ((float)data.price / 100).ToString()}
            });
        }
        ...
        public void OnProductPurchaseFailed(Product product)
        {
            ...
            DataAnalysisMgr.S.CustomEventWithAppsflyer(DataAnalysisDefine.PURCHASE_CANCEL, null, new Dictionary<string, string>()
            {
                {"item_id", data.id},{"value", ((float)data.price / 100).ToString()}
            });
        }
        ...
        public void OnProductRequestFailed(string res)
        {
            ThreadMgr.S.mainThread.PostAction(() =>
            {
                ...
                DataAnalysisMgr.S.CustomEventWithAppsflyer(DataAnalysisDefine.PURCHASE_PRODUCT_FAILED, res);
            });
        }
